...
const YduqsSearchBar = class {
  ...
  createCachedDb() {
    ...
    for (let i = 0; i < allCachedElem.length; i++) {
      ...
      $(allCachedElem[i]).html(utils.removeDoubleSpacesAndBreaks($(allCachedElem[i]).html()));
      ...
      const parsedItems = cloneContents.map(index => {
        ...
        if (cloneContents[index].nodeName === '#text') {
          return utils.cleanString(cloneContents[index].textContent);
        }
  ...
  findInDocument() {
    this.actualFoundItemPosition = -1;
    let txt = utils.cleanString(this.term);
    this.clearResults();
    this.hasSearch = true;
    if (txt === '') {
      return;
    ...
    utils.dispatchEvent('yduqs-search-called', { term: txt });
  }
  /** EVENTS */
  /**
  ...
  clearResults() {
    ...
    utils.dispatchEvent('yduqs-search-cleaned');
  }
  /**
  ...
  navigateOnResults(position) {
    ...
    this.handleClose();
    utils.dispatchEvent('yduqs-search-navigate', { position: this.actualFoundItemPosition });
  }
  handleResultItemClick(position, page) {
    utils.dispatchEvent('yduqs-search-before-navigate', { position, page });
    setTimeout(() => {
