...
export class YduqsModal {
  ...
  render() {
    return (h("div", { class: `c-modal__wrapper ${this._isopen ? 'isopen' : ''}` },
      ...
      h("div", { class: `c-modal ${this.size ? `c-modal--${this.size}` : ''} ${this.variant ? `c-modal__variant--${this.variant}` : ''}` },
        h("div", { class: `c-modal__header ${this._title ? 'c-modal__header--titlered' : ''}` },
          this._title && (h("h3", { class: "c-heading" }, this._title)),
          this.variant === 'lab' ? (h("button", { type: "button", onClick: this.closeModal, class: "c-button c-button__icon-container c-button__icon-small u-text--small p-btn p-open-in-full" },
            h("span", { class: "p-btn c-button__icon material-icons" }, "close"))) : (h("div", { class: "c-modal__close", onClick: this.closeModal },
