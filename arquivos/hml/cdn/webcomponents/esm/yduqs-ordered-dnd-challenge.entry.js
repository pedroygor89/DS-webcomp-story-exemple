...
const YduqsOrderedDndChallenge = class {
  ...
  validateFetchedData(response) {
    if (response) {
      if (!response.info) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `info`"), "."));
      }
      if (!response.sequence || !response.sequence.length) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `sequence`"), "."));
      }
      if (!response.items || !response.items.length) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `items`"), "."));
      }
      if (!response.positiveFeedback) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `positiveFeedback`"), "."));
      }
      if (!response.negativeFeedback) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `negativeFeedback`"), "."));
      }
      if (!response.failFeedback) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `failFeedback`"), "."));
      }
      if (!response.dropzone) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `dropzone`"), "."));
      }
    }
    else {
      this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid `config response`"), "."));
    }
  }
  /**
   * Inicia o desafio
  ...
  componentWillLoad() {
    if (this._id && this.config) {
      ...
      if (!this._id) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid attribute `id`"), "."));
      }
      if (!this.config) {
        this.setError(h("span", null, "yduqs-ordered-dnd-challenge needs a ", h("strong", null, "valid attribute `config`"), "."));
      }
    }
  }
  ...
  renderTip(text, position, btnText, action) {
    return (h("span", { class: `c-lab-game-tutorial-tip ${position}` }, text, Boolean(action) && (h("button", { type: "button", class: `c-button ${Boolean(btnText) ? 'u-text--small' : 'c-button__icon-container c-button__icon-small u-text--small p-btn'}`, onClick: action }, Boolean(btnText) ? btnText : (h("i", { class: "p-btn c-button__icon material-icons" }, "chevron_right"))))));
  }
  getResultThumb(item, index) {
    ...
    return (h("div", { class: `item ${result ? 'success' : 'error'}` }, h("img", { src: item === null || item === void 0 ? void 0 : item.thumb, alt: "" }), h("i", { class: `material-icons icon ${result ? 'success' : 'error'}` }, result ? 'sentiment_very_satisfied' : 'sentiment_dissatisfied')));
  }
  startTutorial() {
    this.showInfo = false;
    this.tutorialItem = 'chances';
  ...
  render() {
    ...
    return (h(Host, { class: `c-lab-game c-ordered c-ordered-challenge step-${this.step}`, ref: ref => (this.hostElem = ref) }, this.step !== _STEP_.LOADING ? (h(Fragment, null, this.step === _STEP_.TUTORIAL && (h("div", { class: "c-lab-game-tutorial-overlay" })), h("header", { class: "c-lab-game-header" }, h("div", { class: "c-lab-game-header-item c-lab-game-tutorial-item c-lab-game-chances" }, [...Array(this.chances)].map((item, index) => (
    ...
    h("i", { class: "material-icons active" }, index + 1 === this.chances - this.usedChances && this.inCountdownDanger ? 'heart_broken' : 'favorite')) : (h("i", { class: "material-icons" }, "favorite_border")))), (this.step === 'tutorial' && this.tutorialItem === 'chances') && this.renderTip(`Você tem ${this.chances} tentativas`, 'bottom', undefined, () => this.handleNextTutorial('chances'))), !this.hideCount && (h("div", { class: "c-lab-game-header-item c-lab-game-tutorial-item c-lab-game-count" }, (_a = this.selecteds) === null || _a === void 0 ? void 0 :
      _a.length, "/", (_b = this.sequence) === null || _b === void 0 ? void 0 :
      _b.length, (this.step === 'tutorial' && this.tutorialItem === 'count') && this.renderTip(`Você precisa arrastar ${(_c = this.sequence) === null || _c === void 0 ? void 0 : _c.length} itens.`, 'bottom', undefined, () => this.handleNextTutorial('count')))), this.time && (h("div", { class: "c-lab-game-header-item c-lab-game-tutorial-item c-lab-game-countdown" }, h("yduqs-countdown", { id: `${this._id}-countdown`, time: this.time * 60, ref: ref => (this.countdownElem = ref) }), (this.step === 'tutorial' && this.tutorialItem === 'time') && this.renderTip('Cuidado para não ultrapassar o tempo.', 'bottom', 'Começar', () => this.handleNextTutorial('time'))))), h("div", { class: "c-lab-game-container" }, h("div", { class: `c-lab-game-info ${this.step === _STEP_.TUTORIAL ? 'noIndex' : ''} ${this.showInfo ? 'show' : 'hide'}` }, h("div", { class: "c-lab-game-info-text", innerHTML: this.info }), this.step === _STEP_.INFO ? (h("div", { class: "btns" }, h("button", { type: "button", style: { marginRight: '15px' }, class: "c-button u-text--small", onClick: () => this.startTutorial() }, "Tutorial"), h("button", { type: "button", class: "c-button u-text--small", onClick: () => this.handleNextTutorial('time') }, "Come\u00E7ar"))) : (h("div", { class: "expand-btn" }, h("button", { type: "button", class: "c-button c-button__icon-container c-button__icon-small u-text--small p-btn", onClick: () => this.showInfo = !this.showInfo }, h("i", { class: "p-btn c-button__icon material-icons" }, this.showInfo ? 'chevron_left' : 'chevron_right'))))), h("div", Object.assign({ class: `c-lab-game-stage ${this.step === _STEP_.TUTORIAL ? 'noIndex' : ''}` }, this.dropAttr), this.dropImage && (h("img", { src: this.dropImage, class: "dropimage", alt: "" })), ((_d = this.selecteds) === null || _d === void 0 ? void 0 : _d.length) > 0 && ((_e = [...this.data]) === null || _e === void 0 ? void 0 : _e.reverse().map(item => {
      const isSelected = this.selecteds.find(sel => sel.id === item.id);
      return isSelected ? h("img", { src: item.image, class: "preview", alt: "" }) : '';
    }))), h("div", { class: "c-lab-game-options" }, h("div", { class: "c-lab-game-options-scroll" }, (_f = this.data) === null || _f === void 0 ? void 0 : _f.map(item => {
    ...
    })))), this.step === _STEP_.FEEDBACK && (h("yduqs-modal", { variant: "lab", size: "small", id: `${this._id}-feedback-modal`, isopen: true }, h("div", { class: "c-lab-game-modal-feedback" }, this.showFeedback === 'positive' ? (h(Fragment, null, h("div", { class: "content" }, h("p", null, this.positiveFeedback)), h("footer", null, h("button", { type: "button", class: "c-button", onClick: () => this.handleCloseFeedback() }, "Continuar")))) : (h(Fragment, null, ((_g = this.selecteds) === null || _g === void 0 ? void 0 : _g.length) > 0 && ((_h = this.selecteds) === null || _h === void 0 ? void 0 : _h.length) === this.sequence.length ? (h(Fragment, null, h("div", { class: "content" }, h("p", null, (this.usedChances + 1 === this.chances) ? this.failFeedback : this.negativeFeedback)), h("div", { class: "result" }, this.selecteds.map((item, index) => this.getResultThumb(item, index))))) : (h("div", { class: "content" }, h("p", null, "Foi por pouco! Voc\u00EA n\u00E3o finalizou a atividade a tempo e perdeu uma vida."), " ", (this.usedChances + 1 === this.chances) && (h("p", null, this.failFeedback)))), h("footer", null, h("button", { type: "button", class: "c-button", onClick: () => this.handleCloseFeedback() }, (this.usedChances + 1 === this.chances) ? 'Voltar para Teoria' : 'Tentar outra vez'))))))))) : (h("yduqs-lab-error", { errors: this.error }))));
  }
};

export { YduqsOrderedDndChallenge as yduqs_ordered_dnd_challenge };
