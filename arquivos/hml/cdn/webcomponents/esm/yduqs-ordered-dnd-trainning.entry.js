...
const YduqsOrderedDndTrainning = class {
  constructor(hostRef) {
    ...
    this.trainningEnd = createEvent(this, "trainningEnd", 7);
    this.error = [];
    this.selecteds = [];
  ...
  validateFetchedData(response) {
    if (response) {
      if (!response.title) {
        this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid `title`"), "."));
      }
      if (!response.info) {
        this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid `info`"), "."));
      }
      if (!response.sequence || !response.sequence.length) {
        this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid `sequence`"), "."));
      }
      if (!response.items || !response.items.length) {
        this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid `items`"), "."));
      }
      if (!response.dropzone) {
        this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid `dropzone`"), "."));
      }
    }
    else {
      this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid `config response`"), "."));
    }
  }
  /**
   * Inicia o desafio
  ...
  componentWillLoad() {
    if (this._id && this.config) {
      ...
      if (!this._id) {
        this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid attribute `id`"), "."));
      }
      if (!this.config) {
        this.setError(h("span", null, "yduqs-ordered-dnd-trainning needs a ", h("strong", null, "valid attribute `config`"), "."));
      }
    }
  }
  reset() {
  ...
  render() {
    ...
    return (h(Host, { class: `c-lab-game c-ordered c-ordered-trainning step-${this.step}`, ref: ref => (this.hostElem = ref) }, this.step !== _STEP_.LOADING ? (h(Fragment, null, h("header", { class: "c-lab-game-header" }, h("div", { class: "c-lab-game-header-item c-lab-game-tutorial-item c-lab-game-count" }, (_a = this.selecteds) === null || _a === void 0 ? void 0 :
      _a.length, "/", (_b = this.sequence) === null || _b === void 0 ? void 0 :
      _b.length)), h("div", { class: "c-lab-game-container" }, h("div", { class: `c-lab-game-info ${this.step === _STEP_.TUTORIAL ? 'noIndex' : ''} ${this.showInfo ? 'show' : 'hide'}` }, h("div", { class: "c-lab-game-info-text", innerHTML: this.info }), this.step === _STEP_.INFO ? (h("div", { class: "btns" }, h("button", { type: "button", class: "c-button u-text--small", onClick: () => this.handleStartGame() }, "Come\u00E7ar"))) : (h("div", { class: "expand-btn" }, h("button", { type: "button", class: "c-button c-button__icon-container c-button__icon-small u-text--small p-btn", onClick: () => this.showInfo = !this.showInfo }, h("i", { class: "p-btn c-button__icon material-icons" }, this.showInfo ? 'chevron_left' : 'chevron_right'))))), h("div", Object.assign({ class: "c-lab-game-stage" }, this.dropAttr), this.dropImage && (h("img", { src: this.dropImage, alt: "" })), ((_c = this.selecteds) === null || _c === void 0 ? void 0 : _c.length) > 0 && ((_d = [...this.data]) === null || _d === void 0 ? void 0 : _d.reverse().map(item => {
      const isSelected = this.selecteds.find(sel => sel.id === item.id);
      return isSelected ? h("img", { src: item.image, class: "preview", alt: "" }) : '';
    }))), h("div", { class: "c-lab-game-options" }, h("span", { class: `c-lab-game-tutorial-tip left-top ${this.step !== _STEP_.INFO && !Boolean(this.feedback) && this.showTip ? 'show' : 'hide'}`, style: { marginTop: '17px' } }, 'Arraste o item em destaque'), h("div", { class: "c-lab-game-options-scroll" }, (_e = this.data) === null || _e === void 0 ? void 0 : _e.map(item => (h("div", { id: item.id, class: `c-lab-game-options-item ${item.id === (actual === null || actual === void 0 ? void 0 : actual.id) ? 'active u-colored-pulse' : ''} ${ /* item.selected ||*/Boolean(this.feedback) ? 'disabled' : ''}`, draggable: /*!item.selected &&*/ !Boolean(this.feedback), onDragStart: event => this.handleDragStart(event), style: { backgroundImage: `url(${item.thumb})` } })))))), this.feedback && (h("yduqs-modal", { size: "small", variant: "lab", isopen: true, id: `${this._id}-feedback-modal` }, h("div", { class: "c-lab-game-modal-feedback" }, h("div", { class: "content" }, h("div", { class: "thumb" }, h("img", { class: "image", src: this.feedback.thumb, alt: "" }), h("span", { class: `material-icons badge ${this.feedback.isPositive ? 'success' : 'error'}` }, this.feedback.isPositive ? 'check_circle' : 'cancel')), h("p", null, this.feedback.text)), h("footer", null, h("button", { type: "button", class: "c-button", onClick: () => this.handleCloseFeedback() }, "Continuar"))))))) : (h("yduqs-lab-error", { errors: this.error }))));
  }
};

export { YduqsOrderedDndTrainning as yduqs_ordered_dnd_trainning };
