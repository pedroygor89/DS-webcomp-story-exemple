...
const YduqsVirtualMap = class {
  constructor(hostRef) {
    ...
    this.virtualMapPointClick = createEvent(this, "virtualMapPointClick", 7);
    this.error = [];
  ...
  handlePointsPosition() {
    ...
    this.items.forEach((item, index) => {
      ...
      if (item.left < 0 || item.left > ((_a = this.bgDimensions) === null || _a === void 0 ? void 0 : _a.width) || item.top < 0 || item.top > ((_b = this.bgDimensions) === null || _b === void 0 ? void 0 : _b.height)) {
        this.setError(h("span", null, "yduqs-virtual-map error: ", h("strong", null, "Item number ", index + 1, " is outside image area"), ". It demands left ", h("strong", null, "0 - ", (_c = this.bgDimensions) === null || _c === void 0 ? void 0 :
          _c.width), " and top ", h("strong", null, "0 - ", (_d = this.bgDimensions) === null || _d === void 0 ? void 0 :
          _d.height), " pixels."));
      }
      /*
  ...
  validateFetchedData(response) {
    if (response) {
      ...
      if (!response.items || !response.items.length) {
        this.setError(h("span", null, "yduqs-virtual-map needs a ", h("strong", null, "valid `items`"), "."));
      }
    }
    else {
      this.setError(h("span", null, "yduqs-virtual-map needs a ", h("strong", null, "valid `config response`"), "."));
    }
  }
  /**
   * Inicia o Mapa
  ...
  componentWillLoad() {
    if (this._id && this.config) {
      ...
      if (!this._id) {
        this.setError(h("span", null, "yduqs-virtual-map needs a ", h("strong", null, "valid attribute `id`"), "."));
      }
      if (!this.config) {
        this.setError(h("span", null, "yduqs-virtual-map needs a ", h("strong", null, "valid attribute `config`"), "."));
      }
    }
  }
  async centralize() {
  ...
  render() {
    var _a, _b, _c;
    return (h(Host, { class: `c-virtual-map ${((_a = this.bgElem) === null || _a === void 0 ? void 0 : _a.clientWidth) < ((_b = this.hostElem) === null || _b === void 0 ? void 0 : _b.clientWidth) ? 'centered' : ''} ${this.step === _STEP_.LOADING ? 'loading' : ''}`, ref: ref => (this.hostElem = ref) }, h("div", { class: "c-virtual-map-bg", ref: ref => (this.bgElem = ref) }, h("svg", { class: "lines", ref: ref => (this.linesElem = ref) }), this.step !== _STEP_.LOADING ? (h(Fragment, null, this.buildLines(), (_c = this.items) === null || _c === void 0 ? void 0 :
      _c.map((point, index) => (h(Fragment, null, h("button", { class: `c-virtual-map-item ${index + 1 === this.selected ? 'actual' : ''}`, style: this.calculatePointPosition(point.left, point.top), onClick: () => this.handlePointClick(point) }, index + 1)))))) : (h("yduqs-lab-error", { errors: this.error })))));
  }
...
export { YduqsVirtualMap as yduqs_virtual_map };
