...
import { dispatchEvent, removeDoubleSpacesAndBreaks, cleanString } from '../../../utils/utils';
...
export class YduqsSearchBar {
  ...
  render() {
    var _a;
    return (h(Host, { class: "c-search-bar" },
      h("form", { class: `c-search-bar-trigger ${this.open ? 'opened' : ''}`, onSubmit: event => this.handleSubmit(event) },
        h("button", { "data-gtm-event-category": 'joana:[[instituicao]]:[[tipo-usuario]]', "data-gtm-event-action": 'barra-busca:conteudo-clique-botao', "data-gtm-event-label": 'voltar', type: "button", class: "c-button c-button__icon-container u-text--small c-search-bar-trigger-back", onClick: () => this.handleReset() },
          h("span", { class: "c-button__icon material-icons" }, "arrow_back")),
        h("div", { class: "c-search-bar-trigger-input-container" },
          h("input", { type: "search", class: "c-search-bar-trigger-input", onInput: event => this.handleInputChange(event), value: this.term, ref: el => (this.elemInput = el), placeholder: "Pesquisar no tema" }),
          this.hasSearch && (h("span", { class: "c-button__icon material-icons", onClick: () => this.handleClear() }, "close"))),
        h("button", { "data-gtm-event-category": 'joana:[[instituicao]]:[[tipo-usuario]]', "data-gtm-event-action": 'barra-busca:conteudo-clique-botao', "data-gtm-event-label": 'buscar', type: "submit", class: "c-button c-button__icon-container u-text--small c-search-bar-trigger-btn" },
          h("span", { class: "c-button__icon material-icons" }, "search"))),
      h("div", { class: `c-search-bar-result ${this.open ? 'opened' : ''}` },
        h("div", { class: "c-search-bar-result-overlay", onClick: () => this.handleClose() }),
        h("div", { class: "c-search-bar-result-container" }, ((_a = this.result) === null || _a === void 0 ? void 0 : _a.length) > 0 ? (h("div", { class: "c-search-bar-result-list" }, this.result.map(item => (h("div", { class: "c-search-bar-result-list-item", onClick: () => this.handleResultItemClick(item.index, item.page) },
          h("div", { class: "item-breadcrumb" }, item.locator),
          h("div", { innerHTML: item.html })))))) : this.hasSearch ? (h("div", { class: "c-search-bar-result-list-empty" }, `Não encontramos nenhum resultado`)) : (h("div", null))))));
  }
  static get is() { return "yduqs-search-bar"; }
  static get states() { return {
    "open": {},
    "term": {},
    "hasSearch": {},
    "cachedDb": {},
    "actualFoundItemPosition": {},
    "result": {}
  }; }
