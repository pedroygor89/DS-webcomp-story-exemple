            ...
            if (btnTryAgain == false) {
                ...
                if (chosenNumber == question_answers[questionNumber]) {
                    ...
                    if (htmlLanguage == 'en-us') {
                        ...
                        var oopsText = 'Opa! Resposta incorreta.';
                        var fbnText = 'Mas não se preocupe, você pode tentar novamente.';
                    }
...
if (htmlLanguage == 'en-us') {
    var nothingChosen = `Oops, you have to choose one option.`;
} else if (htmlLanguage == 'es') {
    var nothingChosen = `Ups, tienes que elegir una opción.`;
} else {
    var nothingChosen = `Ops, você precisa escolher uma alternativa.`;
}
$('body').append(`<yduqs-notification id="question-warning" dismissible type="warning">${nothingChosen}</yduqs-notification>`);