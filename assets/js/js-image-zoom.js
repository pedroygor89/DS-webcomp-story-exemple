function zoomImage(module) {
    this.render(module)
}

zoomImage.prototype.render = function (module) {
    buildZoom(module)
}

function buildZoom (module){
    
        var windowWidth = window.innerWidth;

        if(windowWidth <= 800){

            module.querySelectorAll(`#img-container-zoom`).forEach((el) => {
                var e = el.querySelector('img')
                e.removeAttribute("style")

                var widthImage = e.dataset.width

                e.setAttribute("style", "width:" + widthImage+"px") 

                var js_image_zoom = el.querySelector('.js-image-zoom__zoomed-image')
                if(js_image_zoom)
                    el.removeChild(js_image_zoom)

                var zoomed_area = el.querySelector('.js-image-zoom__zoomed-area')
                if(zoomed_area)
                    el.removeChild(zoomed_area)

                e.classList.add('img-enlargeable')
                ...
                e.addEventListener('click',function(){
                    ...
                    var verificaModalZoom = document.querySelector('.modalZoom')
     
                    if(!verificaModalZoom){
                        modal = $('<div class="modalZoom">').css({
            ...
            document.querySelectorAll('.modalZoom').forEach(m=>{
                if(m)
                    m.remove();
            })
            ...
            module.querySelectorAll(`#img-container-zoom`).forEach((el) => {
                ...
                var options = {
                    ...
                    zoomWidth: e.dataset.zoomWidth,
                    offset: {vertical: e.dataset.vertical, horizontal: e.dataset.horizontal},
                    zoomPosition: e.dataset.zoomPosition,
                    scale: e.dataset.scale
                };
